...
import { getChannels } from "@/utils/supabase/queries/channel";
import {
...
import { getServer, getServerMembers } from "@/utils/supabase/queries/server";
...
export default function ChannelPage({ user }: ChannelPageProps) {
  ...
  const { data: server } = useQuery({
    ...
    queryFn: async () => {
      return await getServer(supabase, serverId as string);
    },
    enabled: !!serverId,
  });
  ...
  const { data: channels } = useQuery({
    ...
    queryFn: async () => {
      return await getChannels(supabase, serverId as string);
    },
    enabled: !!serverId,
  });
  ...
  const { data: filteredMessages, fetchNextPage: filteredFetchNext } =
    useInfiniteQuery({
      queryKey: ["filteredMessages", channelId],
      queryFn: async ({ pageParam = 0 }) => {
        return await getPaginatedMessages(
          supabase,
          channelId as string,
  ...
  const { data: members } = useQuery({
    ...
    queryFn: async () => {
      return await getServerMembers(supabase, serverId as string);
    },
    enabled: !!serverId,
  });
  ...
  const addMessageToCache = useCallback(
    (newMessage: z.infer<typeof DraftMessage>) =>
      addMessageToCacheFn(queryUtils, channelId, members)(newMessage),
    [channelId, members, queryUtils]
  );
  ...
  const updateMessageInCache = useCallback(
    (updatedMessage: z.infer<typeof DraftMessage>) =>
      updateMessageInCacheFn(queryUtils, channelId, members)(updatedMessage),
    [channelId, members, queryUtils]
  );
  ...
  const deleteMessageFromCache = useCallback(
    (messageId: string) =>
      deleteMessageFromCacheFn(queryUtils, channelId)(messageId),
    [channelId, queryUtils]
  );
  ...
  const addReactionToCache = useCallback(
    (messageId: string, reaction: z.infer<typeof MessageReaction>) =>
      addReactionToCacheFn(queryUtils, channelId)(messageId, reaction),
    [channelId, queryUtils]
  );
  ...
  const removeReactionFromCache = useCallback(
    (reactionId: string) =>
      removeReactionFromCacheFn(queryUtils, channelId)(reactionId),
    [channelId, queryUtils]
  );
  ...
  // a necessity.
  useEffect(() => {
    const messageTableChanges = supabase
      ...
      .on(
        ...
        async (payload) => {
          const newMessage = payload.new as z.infer<typeof DraftMessage>;
      ...
      .on(
        'postgres_changes',
        {
          event: 'UPDATE', schema: 'public', table: 'message', filter: `channel_id=eq.${channelId}`
        },
        (payload) => {
          const updatedMessage = payload.new as z.infer<typeof DraftMessage>;
      ...
      .on(
        'postgres_changes',
        {
          event: 'DELETE', schema: 'public', table: 'message', filter: `channel_id=eq.${channelId}`
        },
        (payload) => {
      ...
      return () => {
        supabase.removeChannel(messageTableChanges);
      }
    }, [addMessageToCache, updateMessageInCache, deleteMessageFromCache, supabase, channelId, user.id])

    useEffect(() => {
      const reactionTableChanges = supabase
      .channel('reaction-table-changes')
      .on(
        'postgres_changes',
        {event: 'INSERT', schema: 'public', table: 'reaction', filter: `channel_id=eq.${channelId}`},
        async (payload) => {
          const newReaction = payload.new as z.infer<typeof Reaction>

          if(newReaction.profile_id !== user.id){
            addReactionToCache(newReaction.id, newReaction)
          }
        }
      )
      .on(
        'postgres_changes',
        {event: 'DELETE', schema: 'public', table: 'reaction', filter: `channel_id=eq.${channelId}`},
        async (payload) => {
          const reactionToDelete = payload.old as z.infer<typeof Reaction>
          removeReactionFromCache(reactionToDelete.id)
        }
      )
      .subscribe()

      return () => {
        supabase.removeChannel(reactionTableChanges)
      }
    }, [addReactionToCache, removeReactionFromCache, supabase, channelId, user.id])

  // Store all of the users who are currently "online" (using the app). Here, we just
  // store user IDs.
  const [onlineUsers, setOnlineUsers] = useState<string[]>([]);
  // Define functions to handle how the online users state is mutated when users join
  // and leave the app.
  ...
  useEffect(() => {
    ...
    presenceChannel
      .on('presence', {event: 'join'}, ({newPresences}) => {
        const joiningUserIds = Object.values(newPresences).map(presence => presence.user_id);
      ...
      .on('presence', {event: 'leave'}, ({leftPresences}) => {
        const leavingUserIds = Object.values(leftPresences).map(presence => presence.user_id)
  ...
  const typingText = useMemo(() => {
    ...
    if (typingUsers.length === 1) {
      ...
      return `${typingUser?.display_name} is typing...`;
    }
    ...
    const typingUserNames = typingUsers.map((userId) => {
      ...
      return typingUser?.display_name;
    });
  ...
  useEffect(() => {
    ...
    const typingChannel = supabase.channel(`channel-${channelId}`);
    typingChannel
      .on('broadcast', { event: 'typingStart' }, (payload) => {
        const userId = payload.payload.message;
        setTypingUsers((prev) =>
          prev.includes(userId) ? prev : [...prev, userId]
        );
      })
      .on('broadcast', { event: 'typingEnd' }, (payload) => {
        const userId = payload.payload.message;
  ...
  }, [channelId, isTyping, supabase, user.id]);
  ...
  useEffect(() => {
    ...
    userChangeChannel
      .on('broadcast', { event: 'userStatusChange' }, () => {
      ...
      if (
        ...
        if (draftMessageText.trim() !== "" || !!selectedFile) {
          ...
          sendMessage(supabase, draftMessage, selectedFile)
            // If it succeeds, update the message in the cache and scroll to the end of the message view.
            .then((postedMessage) => {
              updateMessageInCache(postedMessage);
              messageEndRef.current?.scrollIntoView();
            })
            // If it fails, alert the user and revert the message to the previous state.
            .catch(() => {
  ...
  const onReactionAddForMessage = (messageId: string) => (emoji: string) => {
    ...
    addReactionToMessage(supabase, channelId as string, messageId, emoji).catch(
      () => {
  ...
  const onReactionRemoveForMessage = (messageId: string) => (emoji: string) => {
      ...
      page.flatMap((message) =>
        message.id === messageId
          ? message.reactions
              .filter(
                (reaction) =>
                  reaction.reaction === emoji && reaction.profile_id === user.id
              )
    ...
    if (idsToRemove && idsToRemove.length > 0) {
      ...
      removeReactionFromMessage(supabase, messageId, emoji).catch(() => {
        // If it fails, alert the user and revert the reaction to the previous state.
        addReactionToCache(messageId, {
          id: id,
          reaction: emoji,
